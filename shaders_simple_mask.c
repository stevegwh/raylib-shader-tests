...
int main(void)
{
    ...
    // Load the shader
    Shader shader = LoadShader(0, TextFormat("resources/shaders/glsl%i/rotation.frag", GLSL_VERSION));

    // Load and apply the diffuse texture (colour map)
    Texture texDiffuse = LoadTexture("resources/textures/luos/Noise_Gradients/T_Random_29.png");
    ...
    // NOTE: Don't use MATERIAL_MAP_IRRADIANCE, MATERIAL_MAP_PREFILTER or  MATERIAL_MAP_CUBEMAP as they are bound as
    // cube maps
    Texture texMask = LoadTexture("resources/textures/luos/Noise_Gradients/T_Random_53.png");
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        DrawModelEx(model2, (Vector3){-0.5f, 0.0f, 0.0f}, (Vector3){1.0f, 1.0f, 0.0f}, 50, (Vector3){1.0f, 1.0f, 1.0f},
                    WHITE);
